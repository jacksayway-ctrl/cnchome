...
async function main(){
 ...
 await page('adminGrade');const award=q('[data-grade-dailycash="perCase"]');award.value='5000';award.dispatchEvent(new w.Event('input',{bubbles:true}));q('#tm-grade-form').requestSubmit();click('[data-grade-confirm]');
 const weekly=q('[data-weekly-horizontal]');assert.equal(weekly.querySelectorAll('thead th').length,3);assert.equal(weekly.querySelector('[data-grade-week-min="1"]').value,'6');assert.equal(weekly.querySelector('thead').textContent.includes('이상'),false);
 for(let i=3;i<20;i++)click('[data-grade-week-add]');assert.equal(q('[data-grade-week-add]').disabled,true);assert.equal(d.querySelectorAll('[data-grade-week-min]').length,20);q('[data-grade-week-add]').click();assert.equal(d.querySelectorAll('[data-grade-week-min]').length,20);
 for(let i=20;i>3;i--)click('[data-grade-week-remove="'+i+'"]');assert.equal(d.querySelectorAll('[data-grade-week-min]').length,3);
 const threshold=q('[data-grade-week-min="3"]');threshold.value='9';threshold.dispatchEvent(new w.Event('change',{bubbles:true}));assert.equal(q('[data-grade-week-min="3"]').value,'9');threshold.value='8';threshold.dispatchEvent(new w.Event('change',{bubbles:true}));
 assert.equal(d.querySelectorAll('[data-grade-department]').length,3);assert.equal(q('[data-weekly-horizontal] tbody').children.length,1);assert.equal(q('[data-grade-period="weekly"][data-grade-index="2"][data-grade-field="achievement"]').value,'35000');assert.equal(q('[data-grade-period="weekly"][data-grade-index="3"][data-grade-field="achievement"]').value,'40000');assert.equal(q('[data-grade-dailycash="start"]').closest('tbody').children.length,1);assert.equal(d.querySelectorAll('[data-grade-add="daily"]').length,0);
