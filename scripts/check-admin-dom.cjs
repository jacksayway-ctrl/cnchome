...
async function main(){
 ...
 await page('adminGrade');assert.equal(q('#tm-grade-preview-period').value,'aggregate');assert.equal(q('#tm-grade-preview-count').value,'150');assert.ok(!q('#tm-grade-preview-result').textContent.includes('2,844,000원'));q('#tm-grade-preview-form').requestSubmit();assert.match(q('#tm-grade-preview-result').textContent,/2,844,000원/);const aggregateCount=q('#tm-grade-preview-count');aggregateCount.value='105';aggregateCount.dispatchEvent(new w.Event('input',{bubbles:true}));assert.ok(!q('#tm-grade-preview-result').textContent.includes('2,844,000원'));q('#tm-grade-preview-form').requestSubmit();assert.match(q('#tm-grade-preview-result').textContent,/2,005,000원/);aggregateCount.value='150';aggregateCount.dispatchEvent(new w.Event('input',{bubbles:true}));q('#tm-grade-preview-form').requestSubmit();
 ...
 assert.equal(d.querySelectorAll('[data-grade-auto="monthly"]').length,0);assert.equal(q('[data-original-monthly] tbody').children.length,8);assert.match(q('[data-original-monthly]').textContent,/101~110건/);
