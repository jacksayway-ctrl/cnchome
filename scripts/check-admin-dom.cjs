...
async function main(){
 ...
 setAuto('weekly','start',7);assert.equal(q('[data-weekly-horizontal] th').textContent,'7건');assert.equal(q('[data-weekly-horizontal] thead tr').lastElementChild.textContent,'26건');setAuto('weekly','start',6);
 setAuto('monthly','start',71);assert.match(q('#tm-grade-form').textContent,/201/);const monthlyCells=[...q('[data-grade-auto="monthly"]').closest('section').querySelectorAll('tbody tr')].map(row=>row.cells[0].textContent);assert.deepEqual(monthlyCells.slice(0,5),['0','71','81','91','101']);assert.equal(monthlyCells.at(-2),'191');setAuto('monthly','start',61);
 assert.equal(d.querySelectorAll('[data-grade-department]').length,3);assert.equal(q('[data-grade-dailycash="start"]').closest('tbody').children.length,1);assert.equal(d.querySelectorAll('[data-grade-add="daily"]').length,0);
