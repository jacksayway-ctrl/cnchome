(function () {
  ...
  function calculate(event) {
    try {
      ...
      const fingerprint = JSON.stringify({ base: base.amount, allowances: details.map(d => d.allowance), week: weekInMonth ? weekly.allowance : 0 });
      if (autoFingerprint !== null && fingerprint !== autoFingerprint) $('adjustment-reviewed').checked = false;
      autoFingerprint = fingerprint;
      const result = E.payroll({ basic: base.amount, allowances: [...details.map(d => d.achievement), ...details.map(d => d.extra), weekInMonth ? weekly.allowance : 0], deductions, adjustments: delta ? [{ delta, reason: $('adjustment-reason').value }] : [], prepaid: n('prepaid') });
