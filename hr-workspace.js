...
function chrome(){if(!live())return;const el=bridge.root.querySelector('#live-page-status'),page=global.location.hash.slice(1);if(el&&handles(page))el.textContent='직원·급여 DB 연결됨 · 게시한 본인 명세서만 직원에게 표시됩니다.';const btn=bridge.root.querySelector('[data-page="payslips"]');if(btn)btn.hidden=admin();if(el&&!admin()){if(['attendance','sales','as'].includes(page))el.textContent='기존 사용자 기능 · 예시 내역으로 동작 확인 가능';else if(page==='regions')el.textContent='접수 가능지역 참고 화면 · 최신 운영 기준은 관리자에게 확인해 주세요.';else if(page==='home'||page==='myInfo'||!page)el.textContent='본인 직원정보와 게시된 급여 명세서를 확인합니다.';}}
