...
| 접수·실적 달력·접수 상태 A/S | 후속 요청으로 sales_records / sales_events 서버 저장 및 5초 자동 갱신 추가. 테스트 자료는 구분 표시 |
| 출결·급여 검토 등 일부 관리자 예시 업무 | 기존 예시 동작 유지 |
| 급여 계산 검토 | 예시 계산용. 실제 확정·지급 API로 바뀌지 않음 |

기존 테이블과 업무 산식을 유지하고 접수 기록·변경 이력용 테이블을 추가합니다. 기존 DB 설정 `/etc/cnchome/database.json`을 그대로 사용하며 비밀번호는 패키지에 넣지 않습니다.
