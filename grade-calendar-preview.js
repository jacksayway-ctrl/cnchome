(function(root,factory){
  if(typeof module==='object'&&module.exports)module.exports=factory(require('./grade-calendar.js'));
  else root.GradeCalendarPreview=factory(root.GradeCalendar);
  ...
  function create(api){
    ...
    const values=()=>{
      ...
      if(saved){try{C.distribute(month,Number(saved.count),Number(saved.days),Number(saved.hours));return saved;}catch{if(saved.count===''||saved.days===''||saved.hours==='')return saved;}}
    ...
    function ensureMonth(){if(!Object.keys(rows()).some(date=>date.startsWith(month))){const v=values();for(const row of C.distribute(month,Number(v.count),Number(v.days),Number(v.hours)))rows()[row.date]=row;}}
    ...
    function readInputs(){
      ...
      if(count&&days&&hours&&mode==='aggregate')configs[department()+':'+month]={count:count.value,days:days.value,hours:hours.value,mode};
    }
    function collect(){
      ...
      for(const [date,pair] of pairs){if(mode==='aggregate'&&date.startsWith(month))continue;if(pair.count===''&&pair.hours===''){delete rows()[date];continue;}if(pair.count===''||pair.hours==='')throw Error(date+'의 실적과 근무시간을 모두 입력해 주세요.');rows()[date]=C.validateRecord({date,count:Number(pair.count),hours:Number(pair.hours),role:'general'});}
    }
    ...
    function update(calculate=false){
      ...
      try{
        ...
        collect();
        if(mode==='aggregate'){const v=values();for(const id of ['count','days','hours']){const el=root.querySelector('#tm-grade-preview-'+id);if(el&&(el.value===''||!el.checkValidity()))throw Error('정상 실적·근무일·시간을 올바르게 입력해 주세요.');}for(const row of C.distribute(month,Number(v.count),Number(v.days),Number(v.hours)))rows()[row.date]=row;}
    ...
    root.addEventListener('change',event=>{if(['tm-grade-preview-month','tm-grade-preview-role','tm-grade-preview-mode'].includes(event.target.id)){try{if(event.target.id==='tm-grade-preview-month'){collect();if(mode==='aggregate'){const v=values();for(const row of C.distribute(month,Number(v.count),Number(v.days),Number(v.hours)))rows()[row.date]=row;}configs[department()+':'+month]={...values(),mode};save();}readInputs();replace();}catch(error){const target=root.querySelector('#tm-grade-preview-result');if(target)target.textContent=error.message;}}});
    return {html,update};
  }
  return {create};
});
